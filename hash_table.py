...
class LinearProbeHashTable(Generic[T]):
    ...
    def __linear_probe(self, key: str, is_insert: bool) -> int:
        ...
        for _ in range(len(self.table)):  # start traversing
           # position_index = self.table.index(position)
            ##self.probe_max = self.probe_total
            #if probe_max > self.probe_max:
            ...
            if self.table[position] is None:
                ...
                if is_insert:
                    ...
                    raise KeyError(key)  # sothe key is not in
            elif self.table[position][0] == key:  # found key
                return position

            elif self.table[position][0] !=key:
                if collision ==False: #first time collision
                    position = (position+1) % len(self.table)
                    probe_chain += 1
                    self.collision_count +=1
                    self.probe_total += 1
                    collision = True
                elif collision==True: #probing
                    position = (position+1) % len(self.table)
                    probe_chain += 1
                    self.probe_total += 1
    ...
    def is_half_full(self):
        return self.count == (len(self.table)//2)
    ...
    def statistics(self) -> tuple:
        stats = (self.collision_count, self.probe_total, self.probe_max, self.rehash_count)
        return stats
...
class TestLinearProbeHashTable(unittest.TestCase):
    ...
    def test_statistics(self):
        ...
        dictionary = LinearProbeHashTable(31, 10)
        self.assertEqual(dictionary.statistics(), (0, 0, 0, 0), "Dictionary should be empty")

        dictionary = LinearProbeHashTable(31, 2)
        dictionary.__setitem__('sam', 2)
        dictionary.__setitem__('toby', 4)
        dictionary.__setitem__('elliott', 3)
        dictionary.__setitem__('sam', 1)
        self.assertEqual(dictionary.statistics(), (1, 1, 1, 2), "Dictionary should be (1, 1, 1, 2)")

        dictionary = LinearProbeHashTable(100, 3)
        dictionary.__setitem__('sam', 2)
        dictionary.__setitem__('toby', 4)
        dictionary.__setitem__('elliott', 3)
        dictionary.__setitem__('sam', 1)
        dictionary.__setitem__('bec', 2)
        dictionary.__setitem__('toy', 4)
        dictionary.__setitem__('dog', 3)
        dictionary.__setitem__('next', 1)
        dictionary.__setitem__('the', 2)
        dictionary.__setitem__('boy', 4)
        dictionary.__setitem__('bed', 3)
        dictionary.__setitem__('cat', 1)
        self.assertEqual(dictionary.statistics(), (8, 15, 4, 2), "Dictionary should be (8, 15, 4, 2)")
        #probe total must be longer than probe max
        self.assertGreater(dictionary.statistics()[2], dictionary.statistics()[3])
